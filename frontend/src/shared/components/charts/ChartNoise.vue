<template>
  <div class="noise-chart-wrapper">
    <ButtonGroup>
      <ButtonBase :active="overlayData.type === 'references'" @click="showReferenceNoises">
        <BaseIcon icon-key="bi-soundwave"/>
      </ButtonBase>
      <ButtonBase :active="overlayData.type === 'night'" @click="showNightNoise">
        <BaseIcon icon-key="bi-moon-stars"/>
      </ButtonBase>
      <ButtonBase :active="overlayData.type === 'day'" @click="showDayNoise">
        <BaseIcon icon-key="bi-brightness-high"/>
      </ButtonBase>
    </ButtonGroup>
    <h3 class="text-xl">{{ overlayData.headline }}</h3>
    <div class="noise-chart">
      <svg class="chart" :viewBox="`0 0 600 700`" aria-labelledby="title desc" role="img">
        <g :transform="`translate(0, 50)`">
          <!-- Full Sound Pressure Scale-->
          <rect class="main-bar" transform="translate(0, 0)" :x="chartOffsetX" y="0" :width="chartWidth"
                :height="height"
                fill="#444"></rect>

          <!-- Reference Sound Pressure Source-->
          <g class="marker"
             :transform="`translate(${chartOffsetX + markerWidth}, ${height - dBAToHeight(currentDBA)})`">
            <!--          <rect x=0 y="0" :width="markerWidth" :height="10" fill="#000"></rect>-->
            <svg x="0" :y="-16" xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="#fff"
                 class="bi bi-arrow-bar-left" viewBox="0 0 16 16">
              <path
                d="m3.86 8.753 5.482 4.796c.646.566 1.658.106 1.658-.753V3.204a1 1 0 0 0-1.659-.753l-5.48 4.796a1 1 0 0 0 0 1.506z"/>
            </svg>
            <text x="32" y="0" dy="10" class="label">{{ currentDBA.toLocaleString('de-DE') }} dB(A)</text>
          </g>
          <g
            v-for="element in overlayData.data"
            :key="element.label"
            class="bar"
            :transform="`translate(${chartOffsetX}, ${height - dBAToHeight(element.dbStop)})`"
          >
            <rect x="0" y="0" :fill="element.color" :width="chartWidth"
                  :height="dBAToHeight(element.dbStop - element.dbStart)"></rect>
            <text x="10" y="15" dy="15" class="label">{{ element.label }}</text>
          </g>
          ...
          <g
            ...
            :transform="`translate(${chartOffsetX}, ${height - dBAToHeight(referenceDBA)})`"
          >
            <rect x="0" y="-2.5" fill="rgba(0,0,0,.2)" :width="chartWidth" :height="5"></rect>
            <text x="-20" y="0" dy="10" class="label">{{ referenceDBA }} dB(A)</text>
          </g>
        </g>
      </svg>
    </div>
