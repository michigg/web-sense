...
export const AudioCalculationDBConversion = {
  _calcRMS (pressures: Array<number>): number {
     ...
     */
    const sum = (pressures as number[]).reduce((accumulator: number, currentValue: number) => accumulator + Math.pow(currentValue, 2), 0)
  ...
  magnitudeSpectrumToDBSpectrum (spectrum: Array<number>): Array<number> {
     ...
     */
    return spectrum.map(value => AudioCalculationBasicConversion.magnitudeToDB(value))
  },
  dbSpectrumToDB (dbSpectrum: Array<number>): number {
    ...
    const originalMagnitudes = dbSpectrum.map(db => AudioCalculationBasicConversion.dbToMagnitude(db))
  ...
  averageSPL (dbs: Array<number>): number {
     ...
     * @returns db value that is derived from the dbs param
     */
    const powerRatiosSum = dbs.reduce((accumulator, currentDB) => accumulator + AudioCalculationBasicConversion.dbToPowerRatio(currentDB), 0)
    const N = dbs.length
    // Mittelungspegel
