...
const INPUT_TYPE_KEY: InputType = InputType.ACCELEROMETER
const AVAILABLE_ICON_KEY: string = 'bi-phone-flip'
const UNAVAILABLE_ICON_KEY: string = 'bi-phone-flip'
const SENSOR_PATH: string = 'accelerometerSensor'
const isActive = ref(false)
const isAvailable = ref(false)
const isCalibrated = ref(false)
const sensor = ref<Accelerometer | undefined>()
...
export function useAccelerometerSensor() {
  ...
  const start = async (options?: { frequency: number}) => {
    ...
    sensor.value = new Accelerometer(options)
