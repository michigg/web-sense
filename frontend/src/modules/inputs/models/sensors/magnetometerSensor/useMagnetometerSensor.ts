...
const INPUT_TYPE_KEY: InputType = InputType.MAGNETOMETER
const AVAILABLE_ICON_KEY: string = 'bi-phone-flip'
const UNAVAILABLE_ICON_KEY: string = 'bi-phone-flip'
const SENSOR_PATH: string = 'magnetometerSensor'
const isActive = ref(false)
const isAvailable = ref(false)
const isCalibrated = ref(false)
const sensor = ref<Magnetometer | undefined>()
...
export function useMagnetometerSensor() {
  ...
  const start = async (options?: { frequency: number}) => {
    ...
    if (permissionState !== 'granted') {
      throw new Error('MagnetometerSensor kann aufgrund fehlender Berechtigung nicht gestartet werden. Bitte erteilen Sie die Berechtigung.')
    }
    if (sensor.value) {
      stop()
    }
    sensor.value = new Magnetometer(options)
