...
const INPUT_TYPE_KEY: InputType = InputType.GYROSCOPE
const AVAILABLE_ICON_KEY: string = 'bi-phone-flip'
const UNAVAILABLE_ICON_KEY: string = 'bi-phone-flip'
const SENSOR_PATH: string = 'gyroscopeSensor'
const isActive = ref(false)
const isAvailable = ref(false)
const isCalibrated = ref(false)
const sensor = ref<Gyroscope | undefined>()
const currentSensorValue = ref<GyroscopeValues>()
...
export function useGyroscopeSensor() {
  ...
  const start = async (options?: { frequency: number}) => {
    ...
    sensor.value = new Gyroscope(options)
