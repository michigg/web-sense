<template>
  <BaseList class="measurement-preview">
    <li>
      <h2 class="text-2xl">{{ task.steps[0].title }}</h2>
      <MiniMap
        :lat="task.steps[0].results[0].measurements.get('latitude')"
        :lng="task.steps[0].results[0].measurements.get('longitude')"
      />
      <ChartNoise
        v-if="task.steps[0].results[0].measurements.get('dba')"
        :dba="task.steps[0].results[0].measurements.get('dba')"
      />
