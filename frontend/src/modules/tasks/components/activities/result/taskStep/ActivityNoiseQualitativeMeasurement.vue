<template>
  <BaseList class="survey-questions">
    <li
      v-for="taskStepResult of taskStep.results"
      :key="taskStepResult"
      class="survey-question"
    >
      <h3>{{ questionTitle(taskStepResult.metas) }}</h3>
      <p><strong>Frage:</strong><br> {{ questionDescription(taskStepResult.metas) }}</p>
      <p>
        <strong>Antwort:</strong><br>
        <span v-if="questionAnswer(taskStepResult.measurements)">{{ questionAnswer(taskStepResult.measurements) }}</span>
...
const questionTitle = (metas: Map<string, number | string | boolean>) => metas.get('questionTitle')
const questionDescription = (metas: Map<string, number | string | boolean>) => metas.get('question')
const questionAnswer = (measurements: Map<string, number | string | boolean>) => {
  const answers = []
