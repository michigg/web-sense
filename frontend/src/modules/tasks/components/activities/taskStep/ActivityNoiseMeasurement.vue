...
import { SensorTaskStep } from '@/modules/tasks/models/taskStep'
...
const measure = async () => {
  ...
  try {
    ...
    result.addMeta('windowingFunction', micAnalyzer.analysisConfig.windowingFunction)
    result.addMeta('lowestPerceptibleFrequency', micAnalyzer.analysisConfig.lowestPerceptibleFrequency)
    result.addMeta('highestPerceptibleFrequency', micAnalyzer.analysisConfig.highestPerceptibleFrequency)
    ...
    result.addMeasurement('accuracy', Number(location.coords.accuracy.toFixed(2)))
    const analysisResults = AudioCalculations.getResult(analysisData.getAmplitudeSpectrumList(), micSensor.sampleRate, micSensor.bufferSize)
